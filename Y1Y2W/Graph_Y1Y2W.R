...
loc <- paste("/project/meisel/users/bmmclain/Y1Y2W/OutputArchive/",
              "data_",
              args[1],
              ".txt",
              sep="")
...
types <- c(5, 18, 4, 18, 6, 2, 10, 8)
df_types <- colnames(df)
data_present <- c()
i <- 1
j <- 1
if(df_types[i]=="IIIM_Fem_fitness"){
...
h <- hash()
h[["IIIM_Fem_fitness"]] <- "Allele Fitness"
h[["f1_fit"]] <- "Genotype Fitness"
h[["IIIM_fem_h"]] <- "Dom Results"
