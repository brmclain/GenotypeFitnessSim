...
def simu_v2_01():
    ...
    if type(allele_range).__name__ == "float":
        if (allele_range>0) and (allele_range<=1):
            ...
            allele_fitness = np.random.uniform(size = 5,low = -1,high = 1)
    else:
        allele_fitness = np.random.uniform(size = 5,low = -1,high = 1)
...
def main_v2_01():
    ...
    data = p.starmap(simu_v2_01, [() for _ in range (simulations)])
    p.close()
    p.join()
    ...
    #genotype_path = os.path.join(cur_path, 'output3')         # And then concatenates it with the custom file path
    final_data.to_csv(modif_path, sep = '\t', index = None)
...
if __name__ == "__main__":
    ...
    genotype_equil_dict = {'f1': [], 'f2': [], 'f3': [], 'f4': [], 'f5': [], 'f6': [], 'f7': [], 'f8': [], 'f9': [], 'f10': [],
                        'm1': [], 'm2': [], 'm3': [], 'm4': [], 'm5': [], 'm6': [], 'm7': [], 'm8': []}
