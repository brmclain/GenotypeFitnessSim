#This code is property of the University of houston
...
def recursion_seln_Y1Y2W(raw_counts_ref, fitness_ref):    # pass by reference, not copy
    ...
    after_mating = [f1*m1*.5 + f1*m3*.5 + f1*m4*.25 + f2*m1*.25 + f2*m3*.25 + f2*m4*.125 + f3*m1*.125 + f3*m3*.125
                    ...
                    f1*m1*.5 + f1*m2*1 + f1*m4*.25 + f1*m5*.5 + f2*m1*.25 + f2*m2*.5 + f2*m4*.125 + f2*m5*.25 + f3*m1*.25 + f3*m2*.25
...
def genotype_fitness_Y1Y2W(input_array, dominance): # Input_array will be allele_fitness
...
def sex_ratio_Y1Y2W(freqs):   # Function calculates sex ratio given an array of all possible genotypes
    sum_freq = sum(freqs)
...
def simu_Y1Y2W(generations, dominance, initial_freq, allele_range, switches="1111111"):
    ...
    fitness_array, dom_result = genotype_fitness_Y1Y2W(allele_fitness, dominance)         # Returns fitness array of 18 genotypes, only 1 fitness_array per simulation
    
    #This is where the recursion is happening
    for a in range(generations):                                
    
        result_recursion = recursion_seln_Y1Y2W(simulation_results[a-1], fitness_array)   # Takes genotoype frequencies and uses same genotype fitness array
    ...
    allele_freq = allele_freqs_Y1Y2W(final_sim_result)
    sex_ratios = sex_ratio_Y1Y2W(final_sim_result)
...
def main_Y1Y2W(JobID, simulations, generations, processes, dominance, initial_freq, allele_range, switches):
    start = time.time()
    ...
    while(simulations>0):
        if(simulations > section):
            data = list(p.starmap(simu_Y1Y2W, [(generations, dominance, initial_freq, allele_range, switches) for _ in range (section)]))
        else:
            data = list(p.starmap(simu_Y1Y2W, [(generations, dominance, initial_freq, allele_range, switches) for _ in range (simulations)]))
...
if __name__ == "__main__":
    ...
    main_Y1Y2W(JobID, simulations, generations, def_processes, dominance, initial_freq, allele_range, switches)
